...
// Buscar uma pessoa física específica pelo ID
  ...
  try {
    const query = `
      SELECT
        ...
        p_pf.telefone,
        pf.cpf,
        p_emp.nome AS empresa,
        p_emp.id_pessoa AS empresa_id
      FROM
        pessoas_fisicas pf
      JOIN
        pessoas p_pf ON pf.id_pessoa = p_pf.id_pessoa
      LEFT JOIN
        pessoas p_emp ON pf.id_empresa = p_emp.id_pessoa
      WHERE
        pf.id_pessoa = $1
    `;
